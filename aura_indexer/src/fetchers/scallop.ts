...
import type { ScallopObligation, ScallopUserData } from '../types/index.js';

/**
 * Step 1: Find all ObligationKey NFTs owned by the user.
 * Each key links to exactly one Obligation shared object.
 */
async function fetchObligationKeys(userAddress: string): Promise<Array<{ keyObjectId: string; fields: Record<string, unknown> }>> {
  const keys: Array<{ keyObjectId: string; fields: Record<string, unknown> }> = [];
 ...
 */
async function fetchObligationObject(obligationId: string): Promise<Record<string, unknown> | null> {
  ...
  try {
    ...
    if (response.data && response.data.content && response.data.content.dataType === 'moveObject') {
      return response.data.content.fields as Record<string, unknown>;
    }
 ...
 */
export async function fetchScallopData(userAddress: string): Promise<ScallopUserData> {
  console.log('[Scallop] Starting data fetch...');

  const keys = await fetchObligationKeys(userAddress);
  const obligations: ScallopObligation[] = [];
  ...
  for (const key of keys) {
    ...
    const obligationId = (key.fields.obligation_id as string)
      || (key.fields.ownership as Record<string, unknown>)?.of as string
      || null;
